...
def upload_video(filepath: str) -> dict:
    ...
    return {
        "video_id": data["id"],
        "filename": filename,
        "vss_sensor_id": None,  # VSS /files only returns a single id; no separate sensor id
        "status": "uploaded",
    }
...
def enrich_event(video_id: str, start_s: float, end_s: float, behavior: str,
                  track_id: str | None = None) -> str:
    """
    Ask VSS/Cosmos to describe a specific flagged window — INFORMED by
    AlphaChimp's finding, not blind. This is the gated enrichment call:
    only invoked for events that already cleared the priority gate, and
    told exactly what AlphaChimp saw so Cosmos's description is grounded
    rather than a free-form guess that might describe something else
    entirely in the clip.
    """
    behavior_readable = behavior.replace("_", " ")
    track_note = f" (tracked as chimp #{track_id})" if track_id else ""
    question = (
        f"AlphaChimp flagged '{behavior_readable}'{track_note} between "
        f"{start_s:.0f}s and {end_s:.0f}s in this clip. Describe what is "
        f"directly visible during this window in one or two sentences for "
        f"a zoo worker's review. Do not diagnose illness. Do not infer "
        f"persistent identity."
    )

    if USE_MOCK:
        return (
            f"[MOCK] Around {start_s:.0f}s, visible {behavior_readable}-like "
            f"interaction observed between chimpanzees near the enclosure center."
        )

    return ask_followup(video_id, question)
