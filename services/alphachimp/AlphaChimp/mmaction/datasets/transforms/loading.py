...
@TRANSFORMS.register_module()
class LoadRGBFromFileMulti(BaseTransform):
    """Load a RGB image from file.
...
class SampleAVAFrames(SampleFrames):
    ...
    def __init__(self, clip_len, frame_interval=2, test_mode=False,pair=False):
        self.pair = pair
    ...
    def transform(self, results):
        ...
        if self.pair:
            timestamp = results['timestamp_pair']
        else:
            timestamp = results['timestamp']
        ...
        if self.pair:
            results['frame_inds_pair'] = frame_inds
        else:
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            if modality == 'RGB':
                try:
                    ...
                    while not success:
                        try:
                            print('Raw Frame Decode Failed. mmaction/datasets/transforms/loading.py line 1541.', filepath)
                            filepath = osp.join(directory, filename_tmpl.format(results['frame_inds'][np.random.randint(0, len(results['frame_inds']))]))
                            img_bytes = self.file_client.get(filepath)
                            # Get frame with channel order RGB directly.
                            cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                            imgs.append(cur_frame)
...
@TRANSFORMS.register_module()
class RawKeypointDecode(BaseTransform):
    
    def __init__(self,
                 data_path: str,
                 mode: str,
                 **kwargs) -> None:
        self.ann_list = self._load_data(osp.join(data_path,f'{mode}.pkl'))
    ...
    def transform(self, results: dict) -> dict:
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        video_name = results['video_id']
        if results['frame_inds'].ndim != 1:
            results['frame_inds'] = np.squeeze(results['frame_inds'])

        offset = results.get('offset', 0)
        
        results['frame_inds'] = results['frame_inds'] + offset

        keypoint = self.ann_list['keypoint_process'][video_name][results['frame_inds']][...,:2]
        keypoint_raw = self.ann_list['keypoint'][video_name][results['frame_inds']][...,:2]
        keypoint_score = np.minimum(1,self.ann_list['keypoint'][video_name][results['frame_inds']][...,2])

        results['keypoint_process'] = np.transpose(keypoint,(1,0,2,3))
        results['keypoint'] = np.transpose(keypoint_raw,(1,0,2,3))
        results['keypoint_score'] = np.transpose(keypoint_score,(1,0,2))
