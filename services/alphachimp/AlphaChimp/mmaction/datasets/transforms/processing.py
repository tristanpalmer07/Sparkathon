    ...
    def transform(self, results):
        ...
        if not self.lazy:
            ...
            if 'keypoint_process' in results:
                results['keypoint_process'] = self._resize_kps(results['keypoint_process'],
                                                       self.scale_factor)
        else:
            lazyop = results['lazy']
            if lazyop['flip']:
        ...
        if 'bboxes' in results:
            assert not self.lazy
            results['bboxes'] = self._box_resize(results['bboxes'],
                                                    self.scale_factor)

        if 'instances' in results:
            for idx in range(len(results['instances'])):
                if 'bbox' in results['instances'][idx]:
                    results['instances'][idx]['bbox'] = self._box_resize(results['instances'][idx]['bbox'],
                                                    self.scale_factor)
...
class RandomRescale(BaseTransform):
    ...
    def __init__(self, scale_range, interpolation='bilinear'):
        ...
        self.keep_ratio = not isinstance(scale_range[0], tuple)
        self.interpolation = interpolation
    ...
    def transform(self, results):
        ...
        if self.keep_ratio:
            ...
            short_edge = np.random.randint(self.scale_range[0][0],
                                           self.scale_range[1][0] + 1)
            edge_1 = np.random.randint(self.scale_range[0][1],
                                       self.scale_range[1][1] + 1)
            resize = Resize((edge_1, short_edge),
                            keep_ratio=False,
                            interpolation=self.interpolation,
                            lazy=False)
    ...
    def transform(self, results):
        ...
        if not self.lazy:
            if flip:
                ...
                if 'keypoint_process' in results:
                    kp_p = results['keypoint_process']
                    kp_p, __ = self._flip_kps(kp_p, None, 1)
                    results['keypoint_process'] = kp_p
        else:
            lazyop = results['lazy']
            if lazyop['flip']:
