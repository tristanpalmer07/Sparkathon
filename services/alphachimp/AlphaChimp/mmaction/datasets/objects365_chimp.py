...
class Objects365V1Dataset_Chimp(CocoDataset):
...
@DATASETS.register_module()
class Objects365V2Dataset_Chimp(CocoDataset):
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        ann = data_info.pop('instances')
        data_info['gt_bboxes'] = np.stack([item['bbox'] for item in ann], axis=0)
        data_info['gt_bboxes'][:, 0] = data_info['gt_bboxes'][:, 0] / data_info['width']
        data_info['gt_bboxes'][:, 2] = data_info['gt_bboxes'][:, 2] / data_info['width']
        data_info['gt_bboxes'][:, 1] = data_info['gt_bboxes'][:, 1] / data_info['height']
        data_info['gt_bboxes'][:, 3] = data_info['gt_bboxes'][:, 3] / data_info['height']
        labels = np.stack([item['bbox_label'] for item in ann], axis=0)
        data_info['gt_labels'] = np.eye(self.num_classes)[labels]
        data_info['label'] = data_info['gt_labels']
        data_info['label_mask'] = np.ones(labels.shape[0])

        data_info['img_key'] = data_info['img_path']
