...
class DetectionTransformer(BaseDetector, metaclass=ABCMeta):
    ...
    def get_assigned_feat(self,
                batch_inputs: Tensor,
                batch_inputs_pair: Tensor,
                batch_data_samples: SampleList,
                rescale: bool = True) -> Tuple[Tensor, Tuple[Dict, ...], SampleList]:
        img_inputs = torch.cat((batch_inputs, batch_inputs_pair), dim=0)
        img_feats = self.extract_feat(img_inputs)

        batch_data_samples_exp = batch_data_samples * 2
        head_inputs_dict = self.forward_transformer(img_feats,
                                                    batch_data_samples_exp)
        #print(head_inputs_dict)
        assigned_result = self.bbox_head.get_assigned_feat(
            **head_inputs_dict,
            batch_data_samples=batch_data_samples)
        return assigned_result

    def get_predicted_feat(self,
                batch_inputs: Tensor,
                batch_inputs_pair: Tensor,
                batch_data_samples: SampleList,
                rescale: bool = True, conf_threshold = 0.2) -> Tuple[Tensor, Tuple[Dict, ...], SampleList]:
        img_inputs = torch.cat((batch_inputs, batch_inputs_pair), dim=0)
        img_feats = self.extract_feat(img_inputs)
        ...
        head_inputs_dict = self.forward_transformer(img_feats,
                                                    batch_data_samples_exp)
        #print(head_inputs_dict)
        predicted_result = self.bbox_head.get_predicted_feat(
            **head_inputs_dict,
            batch_data_samples=batch_data_samples, conf_threshold=conf_threshold)
        return predicted_result

    def get_predicted_feat_wo_pair(self,
                batch_inputs: Tensor,
                batch_data_samples: SampleList,
                rescale: bool = True, conf_threshold = 0.2) -> Tuple[Tensor, Tuple[Dict, ...], SampleList]:
        img_inputs = batch_inputs
        img_feats = self.extract_feat(img_inputs)

        head_inputs_dict = self.forward_transformer(img_feats, batch_data_samples)
        #print(head_inputs_dict)
        predicted_result = self.bbox_head.get_predicted_feat_wo_pair(
            **head_inputs_dict,
            batch_data_samples=batch_data_samples, conf_threshold=conf_threshold)
        return predicted_result
