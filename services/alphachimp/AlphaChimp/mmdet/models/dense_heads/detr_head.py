...
class DETRHead(BaseModule):
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
        ...
        (labels_list, label_obj_weights_list, label_cls_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, cls_masks_list) = cls_reg_targets
        ...
        cls_masks = torch.cat(cls_masks_list, 0)
        ...
        loss_cls = self.loss_cls(
            cls_scores, labels, obj_weight=label_obj_weights, cls_weight=label_cls_weights, cls_masks=cls_masks, avg_factor=cls_avg_factor)
    ...
    def get_targets(self, cls_scores_list: List[Tensor],
        ...
        (labels_list, label_obj_weights_list, label_cls_weights_list, bbox_targets_list, bbox_weights_list,
         pos_inds_list,
         neg_inds_list, cls_masks_list) = multi_apply(self._get_targets_single,
                                      cls_scores_list, bbox_preds_list,
                                      batch_gt_instances, batch_img_metas)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
        return (labels_list, label_obj_weights_list, label_cls_weights_list, bbox_targets_list,
                bbox_weights_list, num_total_pos, num_total_neg, cls_masks_list)
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        gt_cls_masks = gt_instances.label_mask
        ...
        labels = gt_bboxes.new_full((num_bboxes, self.num_classes + 1),
                                    0,
                                    dtype=torch.long)
        cls_masks = gt_cls_masks.new_full((num_bboxes, ), 1, dtype=torch.float)
        ...
        cls_masks[pos_inds] = gt_cls_masks[pos_assigned_gt_inds].float()
        label_obj_weights = gt_bboxes.new_ones(num_bboxes)
        label_obj_weights[pos_inds] = 1.0
    ...
    def _predict_by_feat_single(self,
        ...
        # exclude background
        # print('CLSSSS', cls_score)

        q_bbox_pred = bbox_pred[:max_per_img].clone()
        cls_feat_module = self.cls_branches[num_dec][:-1]
        reg_feat_module = self.reg_branches[num_dec]

        reference = references.clone().detach().requires_grad_(True)
        bbox_pred_wgrad = (reg_feat_module(hidden_states) + inverse_sigmoid(reference)).sigmoid()
        ...
        if self.loss_cls.use_sigmoid:
            ...
            bbox_pred = bbox_pred[bbox_index]
            bbox_pred_wgrad = bbox_pred_wgrad[bbox_index]
            det_labels = cls_score[bbox_index, 1:]
            hidden_state = hidden_states[bbox_index]
            cls_hidden_state = cls_feat_module(hidden_state)
        ...
        bbox_pred_wgrad = bbox_pred_wgrad
        bbox_pred_wgrad[:, 0::2] = bbox_pred_wgrad[:, 0::2] * img_shape[1]
        bbox_pred_wgrad[:, 1::2] = bbox_pred_wgrad[:, 1::2] * img_shape[0]
        bbox_pred_wgrad[:, 0::2].clamp_(min=0, max=img_shape[1])
        bbox_pred_wgrad[:, 1::2].clamp_(min=0, max=img_shape[0])

        q_bbox_pred[:, 0::2] = q_bbox_pred[:, 0::2] * img_shape[1]
        q_bbox_pred[:, 1::2] = q_bbox_pred[:, 1::2] * img_shape[0]
        q_bbox_pred[:, 0::2].clamp_(min=0, max=img_shape[1])
        q_bbox_pred[:, 1::2].clamp_(min=0, max=img_shape[0])

        results = InstanceData()
        results.bboxes = det_bboxes
        results.bboxes_wgrad = bbox_pred_wgrad
        results.bbox_indexes = bbox_index
        results.query_bboxes = q_bbox_pred
        results.scores = scores
        results.labels = det_labels
        results.feats = hidden_state
        results.cls_feats = cls_hidden_state
        results.references = references[bbox_index]
        return results


    def loss_by_feat_hierarchy(
        self,
        all_layers_cls_scores: Tensor,
        all_layers_bbox_preds: Tensor,
        batch_gt_instances: InstanceList,
        ...
        losses_cls, losses_hierarchy, losses_bbox, losses_iou = multi_apply(
        ...
        # loss from the last decoder layer
        loss_dict['loss_cls'] = losses_cls[-1]
        loss_dict['loss_hierarchy'] = losses_hierarchy[-1]
        loss_dict['loss_bbox'] = losses_bbox[-1]
        loss_dict['loss_iou'] = losses_iou[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_hierarchy_i, loss_bbox_i, loss_iou_i in \
                zip(losses_cls[:-1], losses_hierarchy[:-1], losses_bbox[:-1], losses_iou[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_hierarchy'] = loss_hierarchy_i
            loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
            num_dec_layer += 1
        return loss_dict

    def loss_by_feat_single_hierarchy(self, cls_scores: Tensor, bbox_preds: Tensor,
                            batch_gt_instances: InstanceList,
                            batch_img_metas: List[dict]) -> Tuple[Tensor]:
        """Loss function for outputs from a single decoder layer of a single
        feature level.
