...
class DeformableDETRHead(DETRHead):
    ...
    def _init_layers(self) -> None:
        ...
        if self.mlp_cls:
            ...
            fc_cls.append(Linear(self.embed_dims, self.cls_out_channels - 1))
            fc_cls = nn.Sequential(*fc_cls)
        else:
            fc_cls = Linear(self.embed_dims, self.cls_out_channels - 1)

        if self.mlp_cls:
            fc_obj = []
            for _ in range(self.num_cls_fcs):
                fc_obj.append(Linear(self.embed_dims, self.embed_dims))
                fc_obj.append(nn.ReLU())
            fc_obj.append(Linear(self.embed_dims, 1))
            fc_obj = nn.Sequential(*fc_obj)
        else:
            fc_obj = Linear(self.embed_dims, 1)
        ...
        if self.share_pred_layer:
            ...
            self.obj_branches = nn.ModuleList(
                [fc_obj for _ in range(self.num_pred_layer)])
            ...
            self.obj_branches = nn.ModuleList(
                [copy.deepcopy(fc_obj) for _ in range(self.num_pred_layer)])
            self.reg_branches = nn.ModuleList([
    ...
    def forward(self, hidden_states: Tensor,
        ...
        for layer_id in range(hidden_states.shape[0]):
            references[layer_id] = references[layer_id].requires_grad_(True)
            reference = inverse_sigmoid(references[layer_id])

            # NOTE The last reference will not be used.
            hidden_state = hidden_states[layer_id]
            outputs_class = self.cls_branches[layer_id](hidden_state)
            outputs_obj = self.obj_branches[layer_id](hidden_state)
            outputs_class = torch.cat((outputs_obj, outputs_class), dim=-1)
    ...
    def predict(self,
                ...
                rescale: bool = True, **kargs) -> InstanceList:
        """Perform forward propagation and loss calculation of the detection
        head on the queries of the upstream network.
        ...
        predictions = self.predict_by_feat(
            *outs, batch_img_metas=batch_img_metas, rescale=rescale, hidden_states=hidden_states, references=references)
        return predictions
    ...
    def predict_by_feat(self,
                        ...
                        all_layers_bbox_preds: Tensor,
                        batch_img_metas: List[Dict],
                        rescale: bool = False, hidden_states: Tensor = None, references: List[Tensor] = None) -> InstanceList:
        ...
        references_last = references[-1]

        result_list = []
        for img_id in range(len(batch_img_metas)):
            cls_score = cls_scores[img_id]
            bbox_pred = bbox_preds[img_id]
            hidden_state = hidden_states_last[img_id]
            reference = references_last[img_id]
            img_meta = batch_img_metas[img_id]
            results = self._predict_by_feat_single(cls_score, bbox_pred, hidden_state, reference, img_meta, all_layers_cls_scores.size(0), rescale)
