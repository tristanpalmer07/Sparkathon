...
class ByteTrackerChimp(BaseTracker):
    ...
    def track(self, data_sample: DetDataSample, **kwargs) -> InstanceData:
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            first_det_labels = torch.ones_like(labels[first_det_inds],device=labels.device)
            first_act_labels = act_labels[first_det_inds]
            ...
            second_det_labels = torch.ones_like(labels[second_det_inds],device=labels.device)
            second_act_labels = act_labels[second_det_inds]
            ...
            first_match_act_labels = first_act_labels[valid]
            ...
            first_unmatch_act_labels = first_act_labels[~valid]
            ...
            act_labels = torch.cat(
                (first_match_act_labels, first_unmatch_act_labels), dim=0)
            act_labels = torch.cat((act_labels, second_act_labels[valid]), dim=0)
