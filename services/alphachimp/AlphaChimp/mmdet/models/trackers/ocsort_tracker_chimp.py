...
class OCSORTTrackerChimp(SORTTracker):
    """Tracker for OC-SORT.
    ...
    def init_track(self, id: int, obj: Tuple[torch.Tensor]):
        ...
        bbox = bbox.detach().squeeze(0).cpu().numpy()
    ...
    def track(self, data_sample: DetDataSample, **kwargs) -> InstanceData:
        ...
        labels = data_sample.pred_instances.labels[:,0]
        act_labels = data_sample.pred_instances.labels
        scores = data_sample.pred_instances.scores
        #print(f"score {scores.max()} {scores.min()} {scores.mean()}")
        frame_id = metainfo.get('timestamp', -1)
        if frame_id == -1:
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            labels = torch.ones_like(labels[valid_inds],device=labels.device)
            act_labels = act_labels[valid_inds]
            ...
            det_labels = torch.ones_like(labels[det_inds],device=labels.device)
            act_labels = act_labels[det_inds]
            ...
            match_act_labels = act_labels[valid]
            ...
            unmatch_act_labels = act_labels[~valid]
            ...
            match_act_labels = torch.cat(
                (match_act_labels, unmatch_act_labels[valid]), dim=0)
            ...
            unmatch_act_labels = unmatch_act_labels[~valid]
            ...
            if len(unmatched_track_inds) > 0:
                ...
                ocr_match_act_labels = unmatch_act_labels[valid]
                ...
                ocr_unmatch_act_labels = unmatch_act_labels[~valid]
                ...
                unmatch_act_labels = ocr_unmatch_act_labels
                ...
                match_act_labels = torch.cat(
                    (match_act_labels, ocr_match_act_labels), dim=0)
                match_det_scores = torch.cat(
            ...
            act_labels = torch.cat((match_act_labels, unmatch_act_labels), dim=0)
        ...
        #print(f"frame_id {frame_id} bboxes {bboxes} labels {labels} scores {scores}")
        self.update(
            ids=ids,
            bboxes=bboxes,
            labels=labels,
        ...
        pred_track_instances.labels = act_labels
